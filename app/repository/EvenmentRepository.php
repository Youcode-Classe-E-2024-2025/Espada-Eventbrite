...
WHERE e.owner_id = :owner_id;
    ...
    public function serchByCategory(int $page = 1, int $limit = 2 , array $categories = null): array
    {
        $offset = ($page - 1) * $limit;
        $query = "SELECT e.id as event_id, e.*, u.username as owner, c.*, cat.name as category, cat.icon as icon
                FROM evenments e 
        ...
        if($categories != null){
            ...
            foreach($categories as $category){
                    $query+=" AND e.category_id = :categoryId$count";
                    $stmt->bindValue(":categoryId$count", $category, PDO::PARAM_INT);
            }
            $query+=" AND e.category_id = :categoryId$count";
        }
        $stmt->bindValue(':limit', $limit, PDO::PARAM_INT);
